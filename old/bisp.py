...
sheean_dat = np.loadtxt("snr_sheean_newt.dat")
xaxis_sheean = sheean_dat[:,0]
yaxis_sheean = sheean_dat[:,1]
	...
	for k1 in k_bins:
		for k2 in k_bins[k_bins<=k1]:
			for k3 in k_bins[k_bins<=k2]: #
				if k3 >= abs(k1-k2):
					try:
						if math.isclose(get_theta(k1,k2,k3)+get_theta(k1,k3,k2)+get_theta(k2,k3,k1), 2*np.pi, abs_tol=10e-8):
...
if __name__ == '__main__':
	...
	for Z in np.arange(0.7,2.1,0.1):
		dop_snrs_varkmax += [get_SNR_on_Z(Z,damp=True,Newtonian=False,damp_on_Ptw=False,kmax_zdep=True)]
		dop_snrs_fixedkmax += [get_SNR_on_Z(Z,damp=True,Newtonian=False,damp_on_Ptw=False,kmax_zdep=False)]
	
	#plotting here
	plt.figure(figsize=(8,8))
	plt.plot(np.arange(0.7,2.1,0.1),dop_snrs_varkmax,label="D B SNR - z dep k_max")
	plt.plot(np.arange(0.7,2.1,0.1),dop_snrs_fixedkmax,label='Dop B SNR - fixed k_max')
	plt.xlim(0.6,2)
	#plt.ylim(0,300)
	plt.legend()
	plt.savefig("D_B_SNR_effofkmax.png")
