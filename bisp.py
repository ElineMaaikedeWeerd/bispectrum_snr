...
def get_SNR_on_Z(Z,damp=True,newtonian=False):
    ...
    def P_twiddle(i,k,mu,B1,f):
        return KN1(i,mu,B1,f)**2 * Pm(i,k) + (1/ng_euclid)#*  np.exp(- (1/2) * ((k[i] * mu[i] * sigma)**2)) + (1/ng_euclid)

    def P_tw_damp(i,k,mu,B1,f):
...
if __name__ == '__main__':
    ...
    for Z in np.arange(0.7,2.1,0.1):
        ...
        dop_snrs_damp += [get_SNR_on_Z(Z,damp=True,newtonian=False)]



    plt.figure(figsize=(8,8))
    # plt.plot(np.arange(0.7,2.1,0.1),snrs,label='SNR - B')
    # plt.plot(np.arange(0.7,2.1,0.1),snrs_damp,label='SNR - B - Damped')
    plt.plot(np.arange(0.7,2.1,0.1),dop_snrs_damp,label='SNR- damp - B_dop')
    #plt.plot(np.arange(0.7,2.1,0.1),newt_snrs_nodamp,label='SNR -no damp B_newt')
    plt.xlim(0.6,2)
    # plt.ylim(0,300)
    plt.legend()
    plt.savefig("dop_B_snrs.png")
